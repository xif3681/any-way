...
app.directive('dashboardApp', function (Notifier, courier, AppState, timefilter, kbnUrl,$translate, $translatePartialLoader,T) {
  ...
  return {
    ...
    controller: function ($scope, $rootScope, $route, $routeParams, $location, Private, getAppState) {
      ...
      仪表盘导航栏
       */
      $scope.dashNavs = [{id:1,title:'默认标签'}];
      /*
      添加按钮
       */
      $scope.addTitle = function () {
        let leng = $scope.dashNavs.length;
        $scope.dashNavs.push({id:leng + 1,title:'新标签页'});
      };
      /*
      删除按钮
       */
      $scope.deleteTitle = function (index) {
        $scope.dashNavs.splice(index,1);
      };
