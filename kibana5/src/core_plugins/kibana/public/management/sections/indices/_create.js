...
.controller('managementIndicesCreate', function ($scope, kbnUrl, Private,Notifier,
indexPatterns, es, config, Promise, $translate, $translatePartialLoader,arSafeModal) {
  ...
  const index = $scope.index = {
    ...
    fetchFieldsError: '{{"Loading"|translate}}'
  };


  //tian.yuanfeng@eisoo.com  2016.8.25翻译索引模式間隔 里面的select里面的内容

  index.nameIntervalOptions.display = null;
  ...
  index.nameIntervalOptions[0].display = Hourly;
  index.nameIntervalOptions[1].display = Daily;
  index.nameIntervalOptions[2].display = Weekly;
  index.nameIntervalOptions[3].display = Monthly;
  index.nameIntervalOptions[4].display = Yearly;
  ...
  $scope.createIndexPattern = function () {
    ...
    .then(function (indexPattern) {
      ...
      .then(function (id) {
        if (id) {
          ...
          let msg = '确认覆盖此索引模式？';
          arSafeModal($scope.createIndexPattern1,msg);
        }
      });
    ...
    .catch(function (err) {
      if (err instanceof IndexPatternMissingIndices) {
        let errLocate = $translate.instant('Could not locate any indices matching that pattern. Please add the index to Elasticsearch');
        notify.error(errLocate);
      }
      else notify.fatal(err);
    });
  };

  $scope.createIndexPattern1 = function () {
    // get an empty indexPattern to start
    indexPatterns.get()
    .then(function (indexPattern) {
      // set both the id and title to the index index
  ...
  function updateSamples() {
    ...
    .then(function (existing) {
      ...
      let errPattern = $translate.instant('Pattern does not match any existing indices');
      patternErrors.push(errPattern);
      ...
      if (_.uniq(samples).length !== samples.length) {
        let errInvalid = $translate.instant('Invalid pattern, interval does not create unique index names');
        patternErrors.push(errInvalid);
      } else {
  ...
  $scope.indexDateFields = [];//luo.chunxiang
  $scope.indexTimeField = null;
  function updateFieldList(results) {
    index.fetchFieldsError = results.fetchFieldsError;
    index.dateFields = results.dateFields;
    //luo.chunxiang
    //仅支持基于@timestamp创建索引，即仅支持以日志记录时间发生。其余时间类型字段，暂时屏蔽
    //AR-207 优化多个时间戳解析索引混乱问题
    if (index && index.dateFields) {
      index.dateFields.map((obj,i)=>{
        if (index.dateFields[i] && index.dateFields[i].name === '@timestamp') {
          $scope.indexDateFields[0] = obj;
        }
      });
    }

    index.timeField = $scope.indexDateFields[0];
