...
app.directive('discFieldChooser', function ($location, globalState, config, $route, Private,$translate) {
  ...
  return {
    ...
    link: function ($scope) {
      ...
      $scope.$watchMulti([
        ...
        //luo.chunxiang
        //2017.3.16
        if (columns.indexOf('_source') > -1 && _.size(columns) > 1) {
          $scope.toggle('_source');
        }
      ...
      $scope.vizLocation = function (field) {
        ...
        $scope.vizLocationHref = '#/visualize/create?' + $.param(_.assign($location.search(), {
      ...
      function getFields() {
        ...
        //LUO.CHUNXIANG
        fields.map((obj,i)=>{
          if (obj.displayName && obj.displayName === '@timestamp') {
            fields.splice(i,1);
          }
        });
        return fields;
