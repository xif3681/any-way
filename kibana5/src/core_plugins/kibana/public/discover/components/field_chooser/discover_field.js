...
app.directive('discoverField', function ($compile,$translate,$translatePartialLoader) {
  ...
  return {
    ...
    link: function ($scope, $elem) {
      ...
      const getWarnings = function (field) {
        ...
        if (!field.scripted) {
          if (!field.doc_values && field.type !== 'boolean' && !(field.analyzed && field.type === 'string')) {
            warnings.push(
              $translate.instant('Doc values are not enabled on this field. This may lead to excess heap consumption when visualizing.'));
          }

          if (field.analyzed && field.type === 'string') {
            warnings.push(
              $translate.instant('This is an analyzed string field. Analyzed strings are highly unique and can use a lot of memory to visualize. Values such as foo-bar will be broken into foo and bar.'));
          }

          if (!field.indexed) {
            warnings.push($translate.instant('This field is not indexed and might not be usable in visualizations.'));
          }
        }


        if (field.scripted) {
          warnings.push($translate.instant('Scripted fields can take a long time to execute.'));
        }
      ...
      $scope.warn = $translate.instant('warning');
      $scope.warns = $translate.instant('warnings');
