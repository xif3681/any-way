...
function VisEditor($scope, $rootScope, $route, timefilter, AppState, $location, kbnUrl, $timeout, courier, Private, Promise,
                   $translate, $translatePartialLoader, searchType,getPathInfo,locals) {
  $scope.$on('ganttToParent', function (event, data) {
    $scope.vis.params.arCategoryGantt = data;
    $scope.$broadcast('transferGantt', data);   //可视化父类监听到，广播给左侧的子类
  });
  ...
  const notify = new Notifier({
    location: $translate.instant('Visualization Editor')
  });
  ...
  $scope.visLogTree = 'visLogTree';//树的id
  ...
  $scope.doSave = function () {
    ...
    $state.vis.params.arCategoryGantt = $scope.vis.params.arCategoryGantt;
    /**
     * 状态监控保存
     */
    if ($state.vis.type === 'arMonitor') {
      $state.vis.params.name = $scope.vis.params.name;
      $state.vis.params.color = $scope.vis.params.color;
    }
    ...
    savedVis.save()
      .then(function (id) {
        ...
        if (id) {
          notify.info($translate.instant('Saved Visualization') + ' "' + savedVis.title + '"');
  ...
  function transferVisState(fromVis, toVis, stage) {
    return function () {
      if ($scope.vis.params.ratios !== undefined) {
        $scope.fetch();
      }
      ...
      if (stage && isAggregationsChanged) {
        ...
        $scope.$broadcast('transferGantt', $scope.vis.params.arCategoryGantt);
        $scope.fetch();
      } else {
        $state.save();
      }
