...
//全局配置，如果采用默认均不需要改动
  var config = {
    path: '/plugins/kibana/alarm/laydate-master/', //laydate所在路径
    skin: 'default', //初始化皮肤
...
//生成表格
  Dates.viewtb = (function () {
    var tr, view = [], weeks = [dateSun, dateMon, dateTus, dateWed, dateThr, dateFry, dateSat];
...
//事件
  Dates.events = function () {
    ...
    Dates.on(as.otoday, 'click', function () {
      var now = new Date();
      S('#laydate_hms input')[0].value = now.getHours();
      S('#laydate_hms input')[1].value = now.getMinutes();
      S('#laydate_hms input')[2].value = now.getSeconds();
