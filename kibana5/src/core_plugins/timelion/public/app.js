...
app.controller('timelion', function ($scope, $http, timefilter, AppState, courier, $route, $routeParams, kbnUrl, Notifier, config, $timeout, Private, savedVisualizations,
  ...
  $scope.search = function () {
    ...
    $http.post('../api/timelion/run', {
      ...
      time: _.extend(timefilter.time, {
        interval: changeVaule($scope.state.interval),
        timezone: timezone
      }),
    })
  ...
  function saveSheet() {
    ...
    savedSheet.save().then(function (id) {
      ...
      if (id) {
        notify.info($translate.instant('Saved sheet as') + '"' + savedSheet.title + '"');
  ...
  function saveExpression(title) {
    savedVisualizations.get({type: 'timelion'}).then(function (savedExpression) {
      ...
      savedExpression.save().then(function (id) {
        if (id) notify.info($translate.instant('Saved expression as') + '"' + savedExpression.title + '"');
      });
    });
  };
