...
export default function buildHierarchicalDataProvider(Private, Notifier,$translate) {
  ...
  let notify = new Notifier({
    location: $translate.instant('Pie chart response converter')
  });
  ...
  return function (vis, resp) {
    ...
    if (!firstAgg._next && firstAgg.schema.name === 'split') {
      notify.warning(
        $translate.instant('Splitting charts without splitting slices is not supported. Pretending that we are just splitting slices.'));
    }
