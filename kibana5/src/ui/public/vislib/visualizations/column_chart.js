...
export default function ColumnChartFactory(Private,$translate) {
  ...
  class ColumnChart extends PointSeriesChart {
    ...
    draw() {
      ...
      let errorsStacked;
      if (lang === 'en-us') {
        errorsStacked = `Cannot display ${self._attr.mode} bar charts for multiple data series \
          with a ${self._attr.scale} scaling method. Try 'linear' scaling instead.`;
      } else if (lang === 'zh-cn') {
        errorsStacked = `无法使用${$translate.instant(self._attr.scale)}显示多个数据系列的${$translate.instant(self._attr.mode)}条形图。请尝试使用线性刻度。`;
      } else {
        errorsStacked = `無法使用${$translate.instant(self._attr.scale)}顯示多個資料數列的${$translate.instant(self._attr.mode)}橫條圖。請嘗試使用線性刻度。`;
      }
      return function (selection) {
        selection.each(function (data) {
          ...
          if (
            ...
            throw new errors.StackedBarChartConfig(errorsStacked);
          }
