...
export default function SavedObjectFactory(es, kbnIndex, Promise, Private, Notifier, safeConfirm, indexPatterns,$translate,
  arSafeModalYes,arSafeModalNo,arSafeModal) {
  ...
  function SavedObject(config) {
    ...
    self.save = function (len) {
      ...
      return self.saveSource(body,len);
    };

    //LUOCHUNXIANG
    self.saveP = function (len) {
      ...
      // }
      return self.saveSourceP(body,len);
    };



    // //LUOCHUNXIANG
    // self.saveSourceP = function (source) {
    ...
    //       let confirmMessage = `${message}${self.title}?`;
    //       return arSafeModalYes(confirmMessage).then(
    //         function () {
    ...
    self.saveId = function (len) {
      ...
      //   return self.saveSource(body);
      // }
      return self.saveSource1(body,len);
    };
    ...
    self.saveSource1 = function (source) {
      let finish = function (id) {
        self.id = id;
        return es.indices.refresh({
          index: kbnIndex
    ...
    self.saveSourceOld = function (source) {
      let finish = function (id) {
        self.id = id;
        return es.indices.refresh({
          index: kbnIndex
    ...
    self.saveSource = function (source,len) {
      let finish = function (id) {
        self.id = id;
        return es.indices.refresh({
          index: kbnIndex
      ...
      let ok = function () {
        ...
        //   if (_.get(err, 'origError.status') === 409) {
        //     let confirmMessage = message + ' ' + self.title + '?';
        //     return arSafeModal(confirmMessage).then(
        //       function () {
        //         return docSource.doIndex(source).then(finish);
        //       },
        //       _.noop // if the user doesn't overwrite record, just swallow the error
      ...
      .catch(function (err) {
        ...
        if (_.get(err, 'origError.status') === 409) {
          ...
          if (len > 1) {
            arSafeModal(ok,msg,true);
          } else {
            arSafeModal(ok,msg,false);
          }
