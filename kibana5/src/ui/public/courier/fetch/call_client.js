...
export default function CourierFetchCallClient(Private, Promise, es,as) {
  ...
  function callClient(strategy, requests) {
    ...
    //luochunxiang 存储索引
    let statusIndexs = [];
    if (statuses[0] && statuses[0]._all) {
      statuses[0]._all.map((obj,i)=>{
        statusIndexs.push(obj.index);
      });
    }

    window.localStorage.setItem('statuses_indexs',statusIndexs.join(';'));
    ...
    .then(function (body) {
      ...
      if (strategy.clientMethod === 'msearch') {
        return (esPromise = as(body.arSearchBody));
