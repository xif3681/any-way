...
define(function (require) {
  ...
  module.directive('tmPagination', [function ($translate, $translatePartialLoader) {
    ...
    return {
      ...
      '<span translate>每页</span><select ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions " ng-change="changeItemsPerPage()"></select>' +
      '/<span translate>共</span><strong>{{ conf.totalItems }}</strong><span translate>条</span> ' +
      // '共<strong>{{ conf.totalItems }}</strong>span>&laquo <span translate>条</span> ' +
      '<span translate>跳转至</span><input type="text" ng-model="jumpPageNum" ng-keyup="jumpPageKeyUp($event)"/><span translate>页</span>' +
      '</div>' +
      ...
      '</ul>' +
      '<div class="no-items" ng-show="conf.totalItems <= 0" translate>暂无数据</div>' +
      '</div>',
      replace: true,
      scope: {
        conf: '='
