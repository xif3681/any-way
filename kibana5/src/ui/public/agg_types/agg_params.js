...
import AggTypesParamTypesStartTimesProvider from 'ui/agg_types/param_types/start_times';
import AggTypesParamTypesEndTimesProvider from 'ui/agg_types/param_types/end_times';
...
export default function AggParamsFactory(Private) {
  ...
  let paramTypeMap = {
    ...
    start_times: Private(AggTypesParamTypesStartTimesProvider),
    end_times: Private(AggTypesParamTypesEndTimesProvider),
  ...
  function AggParams(params) {
    AggParams.Super.call(this, {
      ...
      initialSet: params.map(function (config) {
        let type;
        if (config.name === 'field' || config.name === 'start_times' || config.name === 'end_times') {
          type = config.name;
        } else {
          type = config.type;
        }
