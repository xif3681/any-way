...
export default function TermsAggDefinition(Private, $translate) {
  //luochunxiang@eisoo.com
  var titleOption;
  var Descending;
  var Ascending;
  var SortingAscending;
  var aggValid;
  ...
  if (lang === 'zh-cn') {
    ...
    Ascending = '升序';
    aggValid = '- 聚合无效 –';
    SortingAscending = '不推荐在条件聚合中根据计数来做升序排序';
  } else if (lang === 'zh-tw') {
    titleOption = '條件';
    Descending = '降冪';
    Ascending = '昇冪';
    aggValid = '- 聚合無效 –';
    SortingAscending = '不推薦在條件聚合中根據計數來做昇冪排序';
  } else {
    titleOption = 'Terms';
    Descending = 'Descending';
    Ascending = 'Ascending';
    aggValid = '- agg not valid -';
    SortingAscending = 'Sorting in Ascending order by Count in Terms aggregations is deprecated';
  }
  ...
  let orderAggSchema = (new Schemas([
    {
      ...
      name: 'orderAgg',
      title: 'Order Agg',
      aggFilter: ['!percentiles', '!median', '!std_dev', '!terms' ,'!monitors']
    }
  ])).all[0];
    ...
    params: [
      ...
      },
      {
        ...
        controller: function ($scope) {
          ...
          // if ($scope.agg.vis.type.icon === 'fa-tasks') {
          //   $scope.agg.params.orderBy = '_time';
          //   $scope.agg.params.size = 10;
          // }
        ...
        write: function (agg, output) {
           ...
           */
          let arorderAgg = [];
          if (orderAgg && orderAgg._opts && orderAgg._opts.type === 'percentile_ranks') {
            arorderAgg = orderAgg.params.values;
          }
          ...
          if (orderAgg.type.name === 'count') {
            if (dir === 'asc') {
              routeBasedNotifier.warning(SortingAscending);
            }
            order._count = dir;
            return;
          }
           ...
           * 自定义度量---百分等级 tyf
           */
          if (arorderAgg.length) {
            arorderAgg.map((obj, i)=> {
              order[orderAggId + '.' + obj] = dir;
            });
          } else {
