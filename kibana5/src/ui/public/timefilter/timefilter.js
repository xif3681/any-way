...
uiModules
  ...
  .service('timefilter', function (Private, globalState, $rootScope, config) {
    ...
    Timefilter.prototype.get = function (indexPattern) {
      ...
      // if ($rootScope.ratiosMetric) {
      //   if (timefield) {
      //     let bounds = this.getBounds();
      //     filter = {range: {}};
      //     filter.range[timefield.name] = {
      //       gte: bounds.min.valueOf() - (bounds.max.valueOf() - bounds.min.valueOf()),
    ...
    //罗春香 获取时间戳范围Timestamp range

    Timefilter.prototype.getTimestampFange = function () {
      let filter = {};
