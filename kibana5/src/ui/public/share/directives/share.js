...
app.directive('share', function (Private,$translate) {
    ...
    controller: function ($scope, $document, $location, globalState) {
      ...
      this.toggleShortSnapshotUrl = () => {
        ...
        if (this.urlFlags.shortSnapshot) {
          ...
          const snapshotIframe = this.makeUrlEmbeddableMe(this.urls.snapshot);
      ...
      this.copyToClipboard = selector => {
        const notify = new Notifier({
          ...
          location: `${$translate.instant('Share')}${$translate.instant($scope.objectType)}`,
        });
        ...
        try {
          ...
          if (isCopied) {
            notify.info($translate.instant('URL copied to clipboard.'));
          } else {
            notify.info($translate.instant('URL selected. Press Ctrl+C to copy.'));
          }
        } catch (err) {
          notify.info($translate.instant('URL selected. Press Ctrl+C to copy.'));
        }
