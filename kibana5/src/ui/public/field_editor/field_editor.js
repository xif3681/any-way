...
.directive('fieldEditor', function (Private, $sce) {
  ...
  return {
    ...
    controllerAs: 'editor',
    controller: function ($scope, Notifier, kbnUrl, $http, $q,$translate, $translatePartialLoader) {
      let self = this;
      ...
      getScriptingLangs().then((langs) => {
        /**
         * tianyuanfeng 翻译language option
         */
        let langsTrans = [];
        langs.map(function (obj) {
          langsTrans.push($translate.instant(obj));
        });
        self.scriptingLangs = langsTrans;
      ...
      $scope.selfFieldType = $translate.instant(self.field.type);
      ...
      self.fieldFormatTypes.title = $translate.instant(self.fieldFormatTypes.title);
      self.fieldFormatTypes.map(function (obj) {
        obj.title = $translate.instant(obj.title);
      });
      var savedField;
      var deletedField;
      ...
      self.save = function () {
        ...
        .then(function () {
          notify.info(`${savedField} ${self.field.name}`);
      ...
      self.delete = function () {
        ...
        .then(function () {
          ...
          notify.info(`${deletedField} ${field.name}`);
          redirectAway();
        });
      };
      ...
      function initDefaultFormat() {
        ...
        def.title = $translate.instant('- default -');
